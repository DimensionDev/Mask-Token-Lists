...
const MetaMask = (Object.keys(ContractMetadata) as (keyof typeof ContractMetadata)[])
  .filter((key) => {
...
const QuickSwap = QucikSwapTokens.tokens.map(({ name, address, symbol, decimals, logoURI }) => ({
