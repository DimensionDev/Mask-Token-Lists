...
const getRequestURL = (chainId, ids) =>
  `https://openapi.debank.com/v1/token/list_by_ids?chain_id=${chainId}&ids=${ids}`;
...
async function fetchDebankLogoURI(chainId, tokenIds) {
  ...
  for (let i = 0, j = tokenIds.length; i < j; i += DEBANK_REQUEST_LIMIT) {
    ...
    allRequest.push(
      fetch(getRequestURL(CHAINID_MAPPING[chainId], currentIds.join(","))).then(
        (res) => res.json()
      )
    );
  }
