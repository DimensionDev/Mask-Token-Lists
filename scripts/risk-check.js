...
async function riskCheck() {
  ...
  const addresses = chainIdToTokensMapping[chainId].map((item) => item.address).join(',')
...
function caculateRiskRates(item, key) {
  ...
  if (Number.parseInt(res, 10) > safeRatesRequireCheck) {
    console.error('\x1B[33m WARNING! Risk token address:', key, 'please check it in dist chainId:', chainId)
  }
}
